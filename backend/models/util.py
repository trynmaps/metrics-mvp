...
def get_timestamp_or_none(d: date, time_str: str, tz: pytz.timezone):
    return (
        int(get_localized_datetime(d, time_str, tz).timestamp()) if time_str is not None else None
    )
...
def get_intervals(start_time, end_time, interval_length):
    ...
    rounded_start_time = datetime.strptime(start_time, "%H:%M:%S").replace(
        microsecond=0, second=0, minute=0
    )
    rounded_end_time = datetime.strptime(end_time, "%H:%M:%S").replace(
        microsecond=0, second=0, minute=0
    ) + timedelta(hours=1)
    ...
    while rounded_start_time <= rounded_end_time:
        ...
        time_str_intervals.append(
            (
                ...
                if start_day == new_start_time.date()
                else f"{(new_start_time).strftime('%H:%M:%S')}+1",
            )
        )
