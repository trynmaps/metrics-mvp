...
class TripTimesTest(unittest.TestCase):
    def test_trip_times(self):
        ...
        trips = trip_times.get_completed_trip_times(
            trips1[order1], times1[order1], trips2[order2], times2[order2], is_loop=False
        ).tolist()
        ...
        np.testing.assert_equal(
            arrival_times[inverse_order], [6300, np.nan, 12600, 18900, 30510, 66000, np.nan]
        )

        trips = trip_times.get_completed_trip_times(
            np.array([1]), np.array([6000]), np.array([99]), np.array([166000]), is_loop=False
        ).tolist()

        self.assertEqual(trips, [])

        trips = trip_times.get_completed_trip_times(
            np.array([]), np.array([]), np.array([]), np.array([]), is_loop=False
        ).tolist()
        ...
        trips = trip_times.get_completed_trip_times(
            trips1[order1], times1[order1], trips2[order2], times2[order2], is_loop=True
        ).tolist()
        ...
        np.testing.assert_equal(
            arrival_times[inverse_order], [6300, 12600, 15000, 21090, 36000, np.nan, np.nan]
        )
