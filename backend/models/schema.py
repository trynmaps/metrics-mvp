...
def get_percentiles_data(percentiles, percentile_values):
    return [
        {"percentile": percentile, "value": round(value, ROUND_DIGITS)}
...
def get_histogram_data(histogram, bins, bin_size):
    return [
        {"count": round(float(count), ROUND_DIGITS), "binStart": bin, "binEnd": bin + bin_size}
...
class BasicStats(ObjectType):
    ...
    max = Float()
    std = Float()
    percentiles = List(PercentileData, percentiles=List(Float, required=False))
    histogram = List(
        HistogramBin,
    ...
    countRange = Int(min=Float(required=False), max=Float(required=False))
    values = List(Float)
    ...
    def resolve_histogram(values, info, bin_size=None, min=None, max=None):
        if len(values) > 0:
            ...
            )  # math.floor(percentile_values[0] / bin_size) * bin_size
            bin_max = (
                max
                if max is not None
                else math.ceil(percentile_values[-1] / bin_size) * bin_size + bin_size
            )
...
class TripIntervalMetrics(ObjectType):
    ...
    def resolve_arrivals(parent, info):
        return parent["route_metrics"].get_arrivals(
            direction_id=parent["direction_id"], stop_id=parent["end_stop_id"], rng=parent["range"]
        )
...
class TripMetrics(ObjectType):
    ...
    timeRanges = List(TripIntervalMetrics, date_strs=List(String, name="dates"))
    ...
    def resolve_interval(parent, info, date_strs, start_time=None, end_time=None):
        ...
        rng = metrics.Range(dates, start_time, end_time, agency.tz)

        return {"range": rng, **parent}
    ...
    def resolve_timeRanges(parent, info, date_strs):
        ...
        return [
            {"range": metrics.Range(dates, start_time, end_time, agency.tz), **parent}
    ...
    def resolve_byDay(parent, info, date_strs, start_time=None, end_time=None):
        ...
        return [
            {"range": metrics.Range([date], start_time, end_time, agency.tz), **parent}
            for date in dates
        ]
...
class DirectionIntervalMetrics(ObjectType):
    ...
    medianHeadway = Float()
    averageSpeed = Float(units=String(required=False))
    ...
    def resolve_travelTimeVariability(parent, info):
        return round_or_none(
            parent["agency_metrics"].get_travel_time_variability(
                parent["route_id"], parent["direction_id"], parent["range"]
            )
        )

    def resolve_averageSpeed(parent, info, units=constants.MILES_PER_HOUR):
        return round_or_none(
            parent["agency_metrics"].get_average_speed(
                parent["route_id"], parent["direction_id"], parent["range"], units
            )
        )

    def resolve_medianWaitTime(parent, info):
        return round_or_none(
            parent["agency_metrics"].get_median_wait_time(
                parent["route_id"], parent["direction_id"], parent["range"]
            )
        )
    ...
    def resolve_medianHeadway(parent, info):
        return round_or_none(
            parent["agency_metrics"].get_median_headway(
                parent["route_id"], parent["direction_id"], parent["range"]
            )
        )
    ...
    def resolve_onTimeRate(parent, info):
        return round_or_none(
            parent["agency_metrics"].get_on_time_rate(
                parent["route_id"], parent["direction_id"], parent["range"]
            )
        )

    def resolve_numCompletedTrips(parent, info):
        return parent["agency_metrics"].get_num_completed_trips(
            parent["route_id"], parent["direction_id"], parent["range"]
        )

    def resolve_segments(parent, info):
        return parent["agency_metrics"].get_segment_interval_metrics(
            parent["route_id"], parent["direction_id"], parent["range"]
        )

    def resolve_cumulativeSegments(parent, info):
        return parent["agency_metrics"].get_cumulative_segment_interval_metrics(
            parent["route_id"], parent["direction_id"], parent["range"]
        )
...
class RouteIntervalMetrics(ObjectType):
    ...
    def resolve_directions(parent, info):
        ...
        return [
            {**parent, "direction_id": direction_id}
...
class AgencyIntervalMetrics(ObjectType):
    ...
    def resolve_routes(parent, info):
        ...
        return [{"route_id": route_id, **parent} for route_id in route_ids]
...
class AgencyMetrics(ObjectType):
    ...
    route = Field(RouteMetrics, route_id=String(required=True))
    ...
    def resolve_interval(agency_metrics, info, date_strs, start_time=None, end_time=None):
        ...
        return {"range": rng, "agency_metrics": agency_metrics}
