...
class WaitTimesTest(unittest.TestCase):
    ...
    def test_combine_stats(self):
        ...
        stats1 = wait_times.get_stats(np.array([300, 600, 900, 1500, 3600, 7200]))  # 115 min

        stats2 = wait_times.get_stats(
            np.array([30300, 30360, 31800, 32100, 33000, 36000, 39360])
        )  # 151 min

        stats3 = wait_times.get_stats(np.array([]))

        stats4 = wait_times.get_stats(np.array([60300, 63000, 69600]))  # 155 min

        combined = wait_times.combine_stats([stats1, stats2, stats3, stats4])

        self.assertAlmostEqual(
            combined.get_average(),
            (stats1.get_average() + stats2.get_average() + stats4.get_average()) / 3,
            places=3,
        )
