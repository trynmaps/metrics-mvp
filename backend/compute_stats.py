...
all_stat_ids = [combined_stat_id, median_trip_times_stat_id]
    ...
    for (start_time, end_time) in timestamp_intervals:
        if start_time is None or end_time is None:
            ...
            interval_condition = np.logical_and(
                s1_departure_time_values >= start_time, s1_departure_time_values < end_time
            )
            s1_departure_time_values_by_interval.append(
                s1_departure_time_values[interval_condition]
            )
...
def add_headway_and_wait_time_stats_for_route(
    all_stats, timestamp_intervals, route_config, history_df
):
    ...
    for dir_info in route_config.get_direction_infos():
        ...
        for i, stop_id in enumerate(stop_ids):
            ...
            for interval_index, (start_time, end_time) in enumerate(timestamp_intervals):
                ...
                dir_stats = all_stats[combined_stat_id][interval_index][route_id]["directions"][
                    dir_id
                ]
                ...
                if median_wait_time is not None:
                    ...
                    all_stats[median_trip_times_stat_id][interval_index][route_id]["directions"][
                        dir_id
                    ]["medianWaitTimes"][stop_id] = round(median_wait_time, 1)
...
def add_schedule_adherence_stats_for_route(
    all_stats, timestamp_intervals, route_config, history_df, timetable_df
):
    route_id = route_config.id
    ...
    for dir_info in route_config.get_direction_infos():
        ...
        for i, stop_id in enumerate(stop_ids):
            stop_history_df = history_df[
                (history_sid_values == stop_id) & (history_did_values == dir_id)
            ]
            stop_timetable_df = timetable_df[
                (timetable_sid_values == stop_id) & (timetable_did_values == dir_id)
            ]
            ...
            for interval_index, (start_time, end_time) in enumerate(timestamp_intervals):
                ...
                if start_time is not None:
                    interval_comparison_df = interval_comparison_df[
                        interval_comparison_df["DEPARTURE_TIME"] >= start_time
                    ]

                if end_time is not None:
                    interval_comparison_df = interval_comparison_df[
                        interval_comparison_df["DEPARTURE_TIME"] < end_time
                    ]
                ...
                if num_scheduled_departures > 0:
                    on_time_rate = round(
                        np.sum(interval_comparison_df["on_time"]) / num_scheduled_departures, 3
                    )
                    all_on_time_rates[interval_index].append(on_time_rate)

        for interval_index, _ in enumerate(timestamp_intervals):
            dir_stats = all_stats[combined_stat_id][interval_index][route_id]["directions"][dir_id]
            dir_stats["onTimeRate"] = get_median_or_none(
                all_on_time_rates[interval_index], precision=3
            )
...
def add_trip_time_stats_for_route(all_stats, timestamp_intervals, route_config, history_df):
    ...
    for dir_info in route_config.get_direction_infos():
        ...
        for interval_index, _ in enumerate(timestamp_intervals):
            all_stats[combined_stat_id][interval_index][route_id]["directions"][dir_id][
                "tripTimes"
            ] = collections.defaultdict(dict)
            all_stats[median_trip_times_stat_id][interval_index][route_id]["directions"][dir_id][
                "medianTripTimes"
            ] = collections.defaultdict(dict)
        ...
        for stop_id in stop_ids:
            ...
            if is_loop:
                ...
                sorted_arrival_time_values, sorted_arrival_trip_values = trip_times.sort_parallel(
                    arrival_time_values, trip_values
                )
            else:
        ...
        for i in range(0, i_end_index):
            ...
            s1_trip_values_by_interval, s1_departure_time_values_by_interval = filter_departures_by_interval(
                s1_trip_values, s1_departure_time_values, timestamp_intervals
            )
...
def compute_stats(d: date, agency: config.Agency, routes, save_to_s3=True):
    ...
    for route in routes:
        ...
        add_headway_and_wait_time_stats_for_route(
            all_stats, timestamp_intervals, route_config, history_df
        )
        add_schedule_adherence_stats_for_route(
            all_stats, timestamp_intervals, route_config, history_df, timetable_df
        )
    ...
    for stat_id in stat_ids:
        for interval_index, (start_time, end_time) in enumerate(timestamp_intervals):
            ...
            data = {"routes": all_stats[stat_id][interval_index]}
            precomputed_stats.save_stats(
                agency.id, stat_id, d, start_time_str, end_time_str, data, save_to_s3
            )
