...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="comparison",
        action="store_true",
        help="option to compare timetables to actual data - true or false",
    )
    parser.add_argument(
        "--early-min",
        type=int,
        default=1,
        help="number of minutes before schedule defined as early",
    )
    parser.add_argument(
        "--late-min", type=int, default=5, help="number of minutes after schedule defined as late"
    )
    ...
    timetable_df = timetable.get_data_frame(stop_id=stop_id, direction_id=direction_id).sort_values(
        "TIME"
    )
    ...
    timetable_df["scheduled_headway"] = np.r_[
        np.nan, metrics.compute_headway_minutes(timetable_df["TIME"].values)
    ]
    ...
    for row in timetable_df.itertuples():
        ...
        if args.comparison:
            matching_actual_time = (
                datetime.fromtimestamp(row.matching_actual_time, tz).time()
                if not np.isnan(row.matching_actual_time)
                else None
            )
            ...
            if args.verbose:
                prev_actual_time = (
                    datetime.fromtimestamp(row.prev_actual_time, tz).time()
                    if not np.isnan(row.prev_actual_time)
                    else None
                )
                next_actual_time = (
                    datetime.fromtimestamp(row.next_actual_time, tz).time()
                    if not np.isnan(row.next_actual_time)
                    else None
                )
                closest_actual_time = (
                    datetime.fromtimestamp(row.closest_actual_time, tz).time()
                    if not np.isnan(row.closest_actual_time)
                    else None
                )
            ...
            if not row.no_match:
                ...
                headway_delta = (
                    row.matching_actual_headway - row.scheduled_headway
                    if row.scheduled_headway > 0
                    else None
                )
