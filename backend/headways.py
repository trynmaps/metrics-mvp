    ...
    for d in dates:
        ...
        if args.comparison:
            ...
            comparison_df = timetables.match_actual_times_to_schedule(
                df["TIME"].values, timetable_df["TIME"].values
            )
        ...
        for row in df.itertuples():
            ...
            if args.comparison:
                closest_scheduled_time = (
                    datetime.fromtimestamp(row.closest_scheduled_time, tz).time()
                    if not np.isnan(row.closest_scheduled_time)
                    else None
                )

                headway_diff = (
                    row.headway_min - row.closest_scheduled_headway
                    if not np.isnan(row.closest_scheduled_headway)
                    else None
                )
