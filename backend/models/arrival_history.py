...
def from_data_frame(
    agency_id: str, route_id, arrivals_df: pd.DataFrame, start_time, end_time
) -> ArrivalHistory:
    # note: arrival_history module uses timestamps in seconds, but tryn-api uses ms
    return ArrivalHistory(
        agency_id,
        route_id,
        stops_data=make_stops_data(arrivals_df),
...
def make_stops_data(arrivals: pd.DataFrame):
    ...
    if not arrivals.empty:
        ...
        for stop_id, stop_arrivals in arrivals.groupby(arrivals["SID"]):
            ...
            for stop_direction_id, stop_direction_arrivals in stop_arrivals.groupby(
                stop_arrivals["DID"]
            ):
                arrivals_data = []
            ...
            stops_data[stop_id] = {"arrivals": stop_directions_data}
    return stops_data
